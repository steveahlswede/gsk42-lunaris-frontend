...
from .mocks import MOCK_EMBEDDING_DIMENSIONS, MOCK_EMBEDDING_MODEL_NAME, MockClient, MockEmbeddingsClient
...
async def test_remove_content(monkeypatch, search_info):
    ...
    await manager.remove_content(path="foo's bar.pdf")
...
async def test_remove_content_no_docs(monkeypatch, search_info):
    ...
    manager = SearchManager(search_info)
    await manager.remove_content(path="foobar.pdf")
...
async def test_remove_content_only_oid(monkeypatch, search_info):
    ...
    manager = SearchManager(search_info)
    await manager.remove_content(path="foo.pdf", only_oid="A-USER-ID")
