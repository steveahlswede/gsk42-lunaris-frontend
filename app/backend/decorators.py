...
def authenticated(route_fn: Callable[..., Awaitable[T]]) -> Callable[..., Awaitable[T]]:
    """
    ...
    @wraps(route_fn)
    async def auth_handler(**kwargs):
        auth_helper = current_app.config[CONFIG_AUTH_CLIENT]
        try:
            auth_claims = await auth_helper.get_auth_claims_if_enabled(request.headers)
        except AuthError:
            logging.error("Error getting auth claims", exc_info=True)
            abort(403)

        return await route_fn(**kwargs, auth_claims=auth_claims)
