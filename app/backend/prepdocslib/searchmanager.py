...
class SearchManager:
    ...
    async def remove_content(
        self, chat_id: Optional[str] = None, path: Optional[str] = None, only_oid: Optional[str] = None
    ):
        logger.info(
        ...
        async with self.search_info.create_search_client() as search_client:
            while True:
                filter = self._get_filter(chat_id, path)
                max_results = 1000
    ...
    def _get_filter(self, chat_id: Optional[str] = None, path: Optional[str] = None) -> Optional[str]:
        filter = []
        if chat_id is not None:
            filter.append(f"chatId eq '{chat_id}'")
        if path is not None:
            ...
            filter.append(f"sourcefile eq '{path_for_filter}'")
        if filter:
            output = " and ".join(filter)
        else:
            output = None
        return output
