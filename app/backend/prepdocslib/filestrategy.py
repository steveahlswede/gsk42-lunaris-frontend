    ...
    async def run(self):
        ...
        if self.document_action == DocumentAction.Add:
            ...
            async for path in paths:
                ...
                await search_manager.remove_content(path=path)
...
class UploadUserFileStrategy:
    ...
    async def add_file(self, file: File, chat_id: str):
        ...
        if sections:
            await self.search_manager.update_content(sections, chat_id, url=file.url)

    async def remove_file(self, filename: str, oid: str):
        if filename is None or filename == "":
            logging.warning("Filename is required to remove a file")
            return
        await self.search_manager.remove_content(path=filename, only_oid=oid)
